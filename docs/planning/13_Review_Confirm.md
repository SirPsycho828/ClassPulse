...
Counts derived from `RosterMatchResult.summary` plus extraction failures. Each count is color-coded per the design system (green/yellow/red). "Absent" count comes from `unmatchedRosterStudents`.
...
Each row represents one extracted student:
...
- For answer/score issues: the extracted value shown alongside a source image thumbnail (image path only)
...
- For image path: source image thumbnail shown so the teacher can read the paper themselves
...
- "Enter Data" -- opens a manual entry row where the teacher types in scores/answers
...
For image extractions: clicking a cell shows the source image thumbnail alongside the edit field so the teacher can compare.
...
- No image thumbnails to display
...
- Format normalization previews shown for converted values: `"B+" -> 0.88 (87-89% range)`
...
Acceptable because the review process is designed to take under 60 seconds. Persisting partial review state would add complexity for minimal benefit.

## Edge Cases

| Condition | Handling |
|-----------|----------|
| All rows green, nothing to review | Triage section says "Nothing needs review." Confirmed section expanded by default. "Confirm & Analyze" is immediately enabled. |
...
| Two extracted entries matched to same roster student | Conflict flagged as yellow. Teacher picks which entry belongs to the student, excludes the other. |
...
|------|------|------------|
| Image crop regions | Gap | Showing a precise cropped region alongside low-confidence values requires the extraction AI to return bounding box coordinates. The current `ExtractionResult` schema does not include coordinates. For MVP, show the full source image thumbnail (clickable to enlarge) rather than a precise crop. Add bounding boxes post-MVP. |
...
| Bulk actions | Gap | No "confirm all" or "mark all absent" bulk actions. For a class of 30 with 24 green and 3 yellow, this is manageable. If classes were larger, bulk actions would matter. |
...
| Score format display | Assumption | Validated scores display in the original format (e.g., "8/10") with the normalized value in parentheses: "8/10 (80%)". The normalized value is what enters the pipeline. |
| Concurrent editing | Assumption | Only one browser session can edit the review at a time. No real-time collaboration or conflict detection. If two tabs are open, last-write-wins on "Confirm & Analyze." Unlikely in normal usage. |  
